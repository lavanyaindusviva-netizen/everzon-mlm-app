...
export default function EverzonDashboard() {
  ...
  const refreshAll = useCallback(async () => {
    const [u, o, i, p, pr, aph, pwr, cy] = await Promise.all([
      loadKey("ez_users", []),
      loadKey("ez_orders", []),
      loadKey("ez_income", []),
...
/* DASHBOARD TAB                                                        */
/* ==================================================================== */
function DashboardTab({ currentUser, users, orders, income, isAdmin, carry, passwordRequests, onApprovePassword, onRejectPassword, onResetAllData }) {
...
function ManageUserModal({ user, users, isAdmin, deleteError, onClose, onHold, onUnhold, onSaveKyc, onDelete, onMove }) {
...
/* INCOME TAB                                                           */
/* ==================================================================== */
function IncomeTab({ users, orders, setOrders, income, setIncome, carry, setCarry, currentUser, isAdmin }) {
  ...
  const runWeeklyClosing = async () => {
    ...
    setRunning(false);
    setLastRun(
      `Closing done â€” Total Cycle BV â‚¹${totalCycleBV.toLocaleString("en-IN")}, Scale Factor ${scaleFactor.toFixed(2)}, income credited to ${newIncomeEntries.length} distributors. Unmatched BV carried forward for next week.`
    );
  };
